...
class TestCalculatePrice(unittest.TestCase):
    ...
    def test_same_price(self):
        prices = self.fill_dicts_with_prices([10, 1, 1, 6])
        total = calculate_product_sum_with_discount_util(prices)
        self.assertEqual(16, total)

    def test_same_price_2(self):
        prices = self.fill_dicts_with_prices([10, 10, 10, 10])
        total = calculate_product_sum_with_discount_util(prices)
        self.assertEqual(10, total)

    def test_same_price_3(self):
        prices = self.fill_dicts_with_prices([1, 1, 1, 1])
        total = calculate_product_sum_with_discount_util(prices)
        self.assertEqual(1, total)

    def test_some_discount(self):
        prices = self.fill_dicts_with_prices([10, 11, 9, 10])
        total = calculate_product_sum_with_discount_util(prices)
        self.assertEqual(22, total)

    def fill_dicts_with_prices(self, prices: list[int]) -> list[dict]:
        dicts_for_test = []
        for price in prices:
            dict_product_info = {
